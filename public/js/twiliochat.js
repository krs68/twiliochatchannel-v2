var twiliochat = (function() {
  ...
  function handleInputTextKeypress(event) {
    if (event.keyCode === 13) {
      // New Code added 29-Jan-2019
      // tc.currentChannel.sendMessage($(this).val());
      // change the above line
      // if there is any text like http://www.yahoo.com then replace text with
      // <a href="http://www.yahoo.com" target="_blank">yahoo</a>
      var newTxt = null;
      try {
        var currTxt = $(this).val();
        var arr = currTxt.split(' ');
        for (i in arr) {
          if (arr[i].includes(".com")) {
            arr[i] = '<a href="' + arr[i] + '" target="_blank">Link</a>';
          }
        }
        newTxt = arr.join(' ');
      } catch (e) { newTxt = 'ERROR:' + e }
      // end of code change
      tc.currentChannel.sendMessage(newTxt);
